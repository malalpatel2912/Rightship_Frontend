...
export default function EmpOtpVerificationForm({ credentials, onBack, userData }) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const nextAuthCredentials = {
        ...
        otp: otpValue,
        userType: 'employee' // Always employee for this form
      };
      ...
      if (result?.error) {
        const errorMessages = {
          ...
          'Authentication failed': 'Verification failed. Please try again.'
        };

        throw new Error(errorMessages[result.error] || result.error || 'Failed to verify OTP');
      }
      ...
      // Employee users always go to profile
      router.replace('/jobs');
    } catch (error) {
